...
public class Robot extends TimedRobot {
  ...
  public void robotInit() {
   ...
   rearLeftSlaveFx.setInverted(InvertType.FollowMaster);
    ...
    frontLeftMasterFx.configOpenloopRamp(1, 10);
    frontRightMasterFx.configOpenloopRamp(1, 10);
    rearLeftSlaveFx.configOpenloopRamp(1, 10);
    rearRightSlaveFx.configOpenloopRamp(1, 10);
   ...
   sliderTalonFx.setNeutralMode(NeutralMode.Coast);
   ...
   sliderTalonFx.configForwardSoftLimitThreshold(52000, 10);//change from 63400 to 52000
   ...
   sliderTalonFx.config_kP(0, 0.2, 10);//change form .11 to .2
      ...
      case kCustomAuto:
        if (armTalonFx.getSelectedSensorPosition() < 70672) {
          ...
          if (armTalonFx.getSelectedSensorPosition() >= 70672) {
            ...
            Timer.delay(4);
          }else {System.out.println("end of function2 " + armTalonFx.getSelectedSensorPosition());}
          
          if (armTalonFx.getSelectedSensorPosition() <= 200 && sliderTalonFx.getSelectedSensorPosition() <= 200) {
  ...
  public void teleopPeriodic() {
    ...
    m_Drive.arcadeDrive(speed * 0.8, turn * 0.6);
...
//button 5 is ok
    if (operatorContorlBoard.getRawButton(5)) {
      double armTargetPos1 = 42075;//from 43575 to 37729 to 40575 to 42075
      double sliderTargetPos1 = 52289.8;//from 62553 to 49330  changed to 52289.8

      armTalonFx.set(ControlMode.MotionMagic, armTargetPos1);
      Timer.delay(.5);
      sliderTalonFx.set(TalonFXControlMode.MotionMagic, sliderTargetPos1);
    }
//origanall button 3 now button 1
    else if (operatorContorlBoard.getRawButtonPressed(3)) {
    ...
    } else if (operatorContorlBoard.getRawButtonReleased(3)) {
      double humanIntakeArm = 52072; // 51000 to 52072

      double humanIntakeSlider = -32137.08; // -6777 to -33318 to -30318 changed to -32137.08
...
//button 4 is ok
    if (operatorContorlBoard.getRawButtonPressed(4)) {
      intakeRollers.set(0.25);
    } else if (operatorContorlBoard.getRawButtonReleased(4)) {
      intakeRollers.stopMotor();
    } 
    //origanally button 2 mow button 3
    else if (operatorContorlBoard.getRawButtonPressed(2)) {
      intakeRollers.set(-0.25);
    } else if (operatorContorlBoard.getRawButtonReleased(2)) {
      intakeRollers.stopMotor();
    }
    ...
    SmartDashboard.putNumber("Arm Encoder Position", armTalonFx.getSelectedSensorPosition());
    SmartDashboard.putNumber("Arm Velocity", armTalonFx.getSelectedSensorVelocity());
    SmartDashboard.putNumber("Slider Encoder Position", sliderTalonFx.getSelectedSensorPosition());
    SmartDashboard.putNumber("Slider Velocity", sliderTalonFx.getSelectedSensorVelocity());
    SmartDashboard.putNumber("left side drive train ticks per rotation", frontLeftMasterFx.getSelectedSensorPosition());

    SmartDashboard.putNumber("Intake Encoder Position", intakeEncoder.getPosition());
    SmartDashboard.putNumber("Intake Motor Velocity", intakeEncoder.getVelocity());
